...
# Output: Pandas Dataframe
def preprocess_combined_dataset(df, file_name, drop_na=True, drop_feat=True, baseline=False):
    if baseline:
        df = drop_baseline(df)
...
# Output: Pandas Dataframe
def preprocess_combined_datasets_from_folder(folder_path, file_prefix, drop_na=True, drop_feat=True, baseline=False):
    ...
    for csv_file in csv_files:
        ...
        df = preprocess_combined_dataset(df, output_file_name, drop_na=drop_na, drop_feat=drop_feat, baseline=baseline)
...
# Input: Pandas Dataframe
# Output: Pandas Dataframe
def generate_datasets(df, file_prefix='datasets/ALSFRS_R_FIXED/ALSFRS_R_FIXED', drop_na=True, drop_feat=True, baseline=False):
    if baseline:
        df = drop_baseline(df)
...
# Input: String path to the folder containing the csv files, String prefix for the generated csv files
# Output: Pandas Dataframe
def generate_datasets_from_folder(folder_path, file_prefix, drop_na=True, drop_feat=True, baseline=False):
    csv_files = [f for f in os.listdir(folder_path) if f.endswith('.csv')]
    datasets_sizes = []
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            continue
        print(f"Processing {csv_file}")
        datasets_size = generate_datasets(df, file_prefix, drop_na=drop_na, drop_feat=drop_feat, baseline=baseline)
...
os.makedirs(os.path.dirname("datasets/ALL_DATA_UNION/Fixed/"), exist_ok=True)
os.makedirs(os.path.dirname("datasets/ALL_DATA_UNION/Sliding_windows/"), exist_ok=True)
os.makedirs(os.path.dirname("datasets/ALL_DATA_UNION/First_symptoms/"), exist_ok=True)

# Generate datasets and save them to csv files, also outputs a dataframe of the size of the generated datasets
datasets_size_Fixed = generate_datasets_from_folder(fixed_path, 'datasets/ALL_DATA_UNION/Fixed/', drop_na=True, drop_feat=True, baseline=False)
datasets_size_Fixed.to_csv('datasets/ALL_DATA_UNION_Fixed_datasets_size.csv', index=False)

datasets_size_Sliding_Windows = preprocess_combined_datasets_from_folder(sliding_windows_path, 'datasets/ALL_DATA_UNION/Sliding_windows/', drop_na=True, drop_feat=True, baseline=False)
datasets_size_Sliding_Windows.to_csv('datasets/ALL_DATA_UNION_Sliding_windows_datasets_size.csv', index=False)

datasets_size_F_S_Fixed = generate_datasets_from_folder(first_symptoms_path, 'datasets/ALL_DATA_UNION/First_symptoms/', drop_na=True, drop_feat=True, baseline=False)
datasets_size_F_S_Fixed.to_csv('datasets/ALL_DATA_UNION_First_symptoms_datasets_size.csv', index=False)
